...
template <typename data_type>
inline auto widget_factory::is_viewer_registered() const -> bool
{
...
template <typename data_type>
inline auto widget_factory::is_editor_registered() const -> bool
{
...
template <typename data_type>
inline void widget_factory::unregister_viewer()
{
	_viewer_factories.erase(std::type_index(typeid(data_reader<data_type>)).hash_code());
}

template <typename data_type>
inline void widget_factory::unregister_editor()
{
...
inline auto widget_factory::create(data_reader<void> data_reader, std::uint64_t data_hash, std::string_view name) const
	-> std::unique_ptr<viewer>
{
	if(auto found_it = _nested_data_reader_factories.find(data_hash); found_it != _nested_data_reader_factories.end())
	{
...
inline auto widget_factory::create_viewer(std::any data_reader, std::string_view name) const -> std::unique_ptr<viewer>
{
...
template <typename data_type>
auto widget_factory::create(data_writer<data_type> data_writer, std::string_view name) const -> std::unique_ptr<editor>
{
	return widget_factory::create_editor(std::any(std::move(data_writer)), name);
}

inline auto widget_factory::create(data_writer<void> data_writer, std::uint64_t data_hash, std::string_view name) const
	-> std::unique_ptr<editor>
{
	if(auto found_it = _nested_data_writer_factories.find(data_hash); found_it != _nested_data_writer_factories.end())
	{
...
inline auto widget_factory::create_editor(std::any data_writer, std::string_view name) const -> std::unique_ptr<editor>
{
